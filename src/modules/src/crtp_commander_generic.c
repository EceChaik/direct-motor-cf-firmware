...
static void motorDecoder(setpoint_t *setpoint, uint8_t type, const void *data, size_t datalen)
{
  ...
  ASSERT(datalen == sizeof(struct motorPacket_s));

  setpoint->ground_control = true;
  
  uint16_t rawThrust = values->m1;
  if ( rawThrust < MIN_THRUST ) {
    setpoint->motors.m1 = 0;
  } else {
    setpoint->motors.m1 = fminf(rawThrust, MAX_THRUST);
  }

  rawThrust = values->m2;
  if ( rawThrust < MIN_THRUST ) {
    setpoint->motors.m2 = 0;
  } else {
    setpoint->motors.m2 = fminf(rawThrust, MAX_THRUST);
  }

  rawThrust = values->m3;
  if ( rawThrust < MIN_THRUST ) {
    setpoint->motors.m3 = 0;
  } else {
    setpoint->motors.m3 = fminf(rawThrust, MAX_THRUST);
  }

  rawThrust = values->m4;
  if ( rawThrust < MIN_THRUST ) {
    setpoint->motors.m4 = 0;
  } else {
    setpoint->motors.m4 = fminf(rawThrust, MAX_THRUST);
  }
...
struct thrustPacket_s {
  uint16_t thrust;        // uint16_t value
} __attribute__((packed));
